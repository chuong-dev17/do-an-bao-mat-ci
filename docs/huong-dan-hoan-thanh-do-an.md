...
Tài liệu này đi kèm repo mẫu có sẵn workflow GitHub Actions (Semgrep, SonarCloud, Juice Shop smoke). Bạn có thể đọc thêm tóm tắt kiến trúc trong [plan-ci-cd-khong-can-server.md](plan-ci-cd-khong-can-server.md).
...
- (Khuyến nghị) Cài [Docker Desktop](https://www.docker.com/products/docker-desktop/) để chạy OWASP Juice Shop ở máy.
...
`Push code → GitHub Actions chạy → Công cụ scan (Semgrep / Sonar) → Kết quả (log, báo cáo, Code Scanning) → (Tùy chọn) Test với Juice Shop local hoặc job smoke trên CI`
...
git init
git add .
git commit -m "Initial: workflows Semgrep, SonarCloud, Juice Shop smoke"
git branch -M main
...
## Phần 5 — OWASP Juice Shop (môi trường lab)

### Bước 5.1: Chạy Juice Shop trên máy (Docker)

```bash
docker run --rm -p 3000:3000 bkimminich/juice-shop
```

Mở trình duyệt: `http://localhost:3000`. Dùng cho **thực hành lỗ hổng**, **ghi chép kịch bản**, **ảnh màn hình** trong báo cáo.

### Bước 5.2: Minh chứng trên CI (đã có sẵn)

Workflow [**Juice Shop smoke (service container)**](../.github/workflows/juice-shop-smoke.yml) chạy khi push lên `main` / `master` / `develop`, hoặc chạy tay: Actions → workflow đó → **Run workflow**.

Trong báo cáo, giải thích: đây là bước **kiểm tra target động** trong pipeline (container + `curl`), **không thay thế** pentest đầy đủ.
...
- **Semgrep** và **Sonar** đang quét toàn repo (Semgrep) và thư mục `src` (Sonar theo `sonar.sources=src`).
- Nếu đồ án là web/API, hãy đặt mã chính dưới `src/` (hoặc sửa `sonar.sources` và exclusions trong `sonar-project.properties` cho khớp).
...
| Log scan | Vào từng run → mở job → copy đoạn log Semgrep / Sonar / Juice Shop |
| Số lỗi / cảnh báo | Log Semgrep; màn hình SonarCloud Issues; Code scanning (nếu có) |
| Thời gian | GitHub hiển thị duration mỗi job |
| Juice Shop | Ảnh giao diện + mô tả 1–2 bài lab (theo yêu cầu môn) |
...
3. **Công cụ**: GitHub Actions, Semgrep, SonarCloud, Juice Shop — vai trò từng cái.
...
- [ ] Đã chạy Juice Shop local hoặc giải thích rõ job smoke trên Actions.
- [ ] Báo cáo có **ảnh pipeline**, **trích log**, **số finding / thời gian** theo yêu cầu thầy/cô.
- [ ] Đã nêu rõ: Semgrep/Sonar phân tích **mã trong repo**, không tự động thay thế đánh giá toàn bộ ứng dụng Juice Shop trừ khi có thêm công cụ/kịch bản DAST.
...
| Juice Shop smoke timeout | Lần đầu pull image có thể lâu; có thể tăng vòng lặp chờ trong workflow hoặc chạy lại. |
...
| [`.github/workflows/juice-shop-smoke.yml`](../.github/workflows/juice-shop-smoke.yml) | Juice Shop trong CI |
| [`sonar-project.properties`](../sonar-project.properties) | Cấu hình Sonar |
| [`src/main.py`](../src/main.py) | Mã mẫu để scan (thay bằng mã đồ án của bạn) |
